...
def search_metrics_files(parent_dir, metric_folder):
    # search_items = ["cd_icp", "f10_icp", "f5_icp", "cd_icp_no_scale", "f10_icp_no_scale", "f5_icp_no_scale", "mpjpe_ra_r", "scale", "cd_right"]
    search_items = ["cd_icp", "f10_icp", "f5_icp"]
    results = {}
            ...
            try:
                ...
                if metrics_idx >= 2:
                    ...
                    try:
                        ...
                        if sequence_dir not in results:
                            results[sequence_dir] = {}
                        if method_dir not in results[sequence_dir]:
                            results[sequence_dir][method_dir] = {}
                        ...
                        results[sequence_dir][method_dir] = metrics_dict
...
def create_results_file(results, search_items, output_path):
    sorted_sequences = sorted(results.keys(), key=natural_sort_key)
    
    with open(output_path, 'w') as f:
        for sequence in sorted_sequences:
            f.write(f"Table for {sequence}\n")
            
            df = pd.DataFrame.from_dict(results[sequence], orient='index')
            df = df[search_items]
            
            # sorted_index = sorted(df.index, key=natural_sort_key)
            # sorted by cd_icp
            df = df.sort_values(by="cd_icp", ascending=False)
            ...
            avg_line = "Avg" + " "*(len(df.index.name or '') + 2)
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help='Parent directory containing evaluation results'
    )
    parser.add_argument(
        '--metric_folder',
        type=str,
